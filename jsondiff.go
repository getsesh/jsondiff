...
func Compare(a, b []byte, opts *Options) (Difference, string) {
	...
	if errA != nil && errB != nil {
		return BothArgsAreInvalidJson, fmt.Sprintf("both arguments are invalid json: result: %s\n\nexpected: %s", errA, errB)
	}
	if errA != nil {
		return FirstArgIsInvalidJson, fmt.Sprintf("first argument is invalid json: %s", errA)
	}
	if errB != nil {
		return SecondArgIsInvalidJson, fmt.Sprintf("second argument is invalid json: %s", errB)
	}
